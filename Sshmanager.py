...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر در حالت انتظار برای حذف کاربر هستیم
    if context.user_data.get("awaiting_delete"):
        username = text

        # بررسی وجود کاربر
        try:
            pwd.getpwnam(username)  # اگر کاربر وجود نداشته باشد، اینجا خطا می‌دهد
        except KeyError:
        ...
        try:
            ...
            await update.message.reply_text("❌ خطا در بررسی UID کاربر.")
            context.user_data["awaiting_delete"] = False
            return
